...
def run(cfg: ModulusConfig) -> None:
    ...
    # # inferencers (prediction at fixed times) using Npts points in space
    # x_inf = np.expand_dims(np.linspace(prb.x_start, prb.x_end, npts), axis=-1)
    # # create inferencers in loop from t=0 to tmax with steps=dt_infer
    # t_infer = np.arange(prb.tstart, prb.tmax+dt_infer, dt_infer)
    # for i in range(0, t_infer.size):
    #     t_inf_loop = np.full(np.shape(x_inf), t_infer[i]) 
    #     label = f"inf_t_{t_infer[i]:.2f}"       
    #     inf_loop = PointwiseInferencer(
    #         nodes=nodes,
    #         invar={"x": x_inf, "t": t_inf_loop},
    #         output_names=["h", "u"],
    #         batch_size=100,
    #     )
    #     domain.add_inferencer(inf_loop, label)

    # # inferencers (timeseries at fixed point) using Npts steps in time (can be modified to loop if necessary)
    # t_discr = np.expand_dims(np.linspace(prb.tstart, prb.tmax, npts), axis=-1)
    # x_point = np.full(np.shape(t_discr), x_p1)
    # inferP1 = PointwiseInferencer(
    #     nodes=nodes,
    #     invar={"x": x_point, "t": t_discr},
    #     output_names=["h", "u"],
    #     batch_size=100,
    # )
    # domain.add_inferencer(inferP1, "inf_point1")
    ...
    t_mon = np.full(np.shape(x_mon), prb.tmax)
