...
def modify_config_file(basefile, name_of_test):
    ...
    doc["batch_size"]["IC"] = 400    
    doc["batch_size"]["BC"] = 200 
    doc["batch_size"]["interior"] = 2000
...
test_name = "bumpsteady"      # name of the test
model_name = "run_bumpsteady"    # name of the script with the solver
...
# instantiate problem
prb = Bump("B1")                                              # with predefined values (B0-B1)
...
dim_time_val = 100    # time at which we have validation data (profile along x axis)
...
dim_dt_infer = 20  # time interval to plot profiles as inferencers 
