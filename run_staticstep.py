...
from swe_utils.riemann_step_problems import create_valdata_from_analytical_step
...
def run(cfg: ModulusConfig) -> None:
    ...
    hin = prb.values[0] - bed_elev #Piecewise((prb.values[0], x < prb.xdisc),(prb.values[2], True))
    uin = 0 #Piecewise((prb.values[1], x < prb.xdisc),(prb.values[3], True))
    z_bound_L = prb.values[4]     
    z_bound_R = prb.values[5]     
    h_bound_L = prb.values[0]
    h_bound_R = prb.values[2]
    u_bound_L = 0 #prb.values[1]
    u_bound_R = 0 #prb.values[3]
