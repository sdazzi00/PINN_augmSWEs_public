import yaml, os
from swe_utils.small_pert_problems import SmallPert
from swe_utils.scales import Scales, nondimensionalize_param
from swe_utils.func_paths import create_paths
from swe_utils.fv_solver import create_fv_sol_with_z, create_ics_sp
...
def modify_config_file(basefile, name_of_test):
    ...
    doc["custom"]["weights"]["zbed"] = 100
    doc["custom"]["weights"]["obs"] = 10
    ...
    doc["training"]["max_steps"] = 15000     # example to modify value of existing param
    doc["training"]["rec_results_freq"] = 2000 
    doc["batch_size"]["IC"] = 100    
    doc["batch_size"]["BC"] = 50
    doc["batch_size"]["interior"] = 500
    ...
    doc["batch_size"]["observations"] = 10
...
# SETUP OF THE RIEMANN PROBLEM (RP)

test_name = "OBS_B"      # name of the test
model_name = "run_OBS_B"    # name of the script with the solver
...
# instantiate Small Pert Problem
prb = SmallPert("SP1")                                                     # with predefined values (SP0-SP1)
#prb = SmallPert(dim_vals=[0.8,0.2,0.4,0.],L=20.,xdisc=0.,tmax=5.)     # alternatively: with custom values
prb.print_recap()
...
dim_dt_infer = 0.5  # time interval to plot profiles as inferencers 
nsteps = 5          # number of profiles to be extracted for validation
npts = 1200          # number of points to discretize the profile of validators
x_p1 = 0.           # location for inferencing time series
# create paths to validation data
valdata_abspath = sol_dir + filename_valdata
# run fv solver to create the validation data at fixed time steps (dimensional)
dx = prb.L / npts 
x_in, h_in, u_in, q_in, z_in = create_ics_sp(prb, dx)
...
nd_params = Scales([1.,1.,1.,1.], nd_par_hypothesis = False, g=9.81)  
print(nd_params.L0, nd_params.H0, nd_params.U0, nd_params.T0)
print(nd_params.Str, nd_params.Fr0_sq)
