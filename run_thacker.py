...
from swe_utils.thacker_problems import create_thacker_planar_sol_from_analytical, read_val_profile_from_csv_and_nondim, create_val_profile_bottom_thacker
...
def run(cfg: ModulusConfig) -> None:
    ...
    bed_elev = prb.values[1] * ( (x/prb.values[0])**2 - 1. )
    hin = Piecewise(
        (0., x < -0.5-prb.values[0]),
        (0., x > -0.5+prb.values[0]),
        ( -prb.values[1] * ( 1./prb.values[0]**2 * (x+ 0.5)**2 - 1. ), True )
    )
    uin = 0
    z_bound_L = prb.values[1]*((0.5*prb.L/prb.values[0])**2 -1)
    z_bound_R = z_bound_L
    h_bound_L = 0
    h_bound_R = 0
    u_bound_L = 0
    ...
    BC_L = PointwiseBoundaryConstraint(
        ...
        lambda_weighting={"h": cfg.custom.weights.bc_coef2, 
                          "u": cfg.custom.weights.bc_coef2,
                          "z": cfg.custom.weights.zbed },
    )
    ...
    for i in range(0, t_val_loop.size):
        ts_val = t_val_loop[i] 
        xa, ta, ha, ua, qa, za = create_thacker_planar_sol_from_analytical(prb, ts_val, npts)
    ...
    # z1 = create_val_profile_bottom_thacker(prb, x1, nd_params)
    ...
    t_mon2 = np.full(np.shape(x_mon), 0)
    global_monitor2 = PointwiseMonitor(       
        invar={"x": x_mon, "t": t_mon2},
        output_names=["h"],
        metrics={
            "mass_balance0": lambda var: torch.sum(
                0.001 * prb.L * torch.abs(var["h"])
            ),           
        },
        nodes=nodes,
    ...
    domain.add_monitor(global_monitor2)
