import yaml, os
from swe_utils.thacker_problems import Thacker
...
def modify_config_file(basefile, name_of_test):
    ...
    doc["training"]["max_steps"] = 60000     # example to modify value of existing param
    doc["training"]["rec_results_freq"] = 2000 
    doc["batch_size"]["IC"] = 2500    
    doc["batch_size"]["BC"] = 500 
    doc["batch_size"]["interior"] = 12000
    doc["optimizer"]["lr"] = 0.001
    doc["scheduler"]["decay_steps"] = 500
...
test_name = "thacker"      # name of the test
model_name = "run_thacker"    # name of the script with the solver
...
# instantiate problem
prb = Thacker("T1")                                              # with predefined values (B0-B1)
#prb = Thacker(dim_vals=[1.,0.5,0.],L=4.,xdisc=0.,tmax=4.)    # alternatively: with custom values
prb.print_recap()
