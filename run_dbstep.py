...
from swe_utils.riemann_step_problems import create_valdata_from_analytical_step, create_val_profile_bottom_step, read_val_profile_from_csv_and_nondim
...
def run(cfg: ModulusConfig) -> None:
    ...
    # validator loop using the in-house functions for exact solution of RP with step
    step = (prb.tmax - prb.tstart) / (nsteps-1)
    t_val_loop = np.arange(prb.tstart, prb.tmax+step, step)
    for i in range(0, t_val_loop.size):
        ts_val = t_val_loop[i] 
        xa, ta, ha, ua, qa, za = create_valdata_from_analytical_step(prb, ts_val, npts)
    ...
    # z1 = create_val_profile_bottom_step(prb, x1, nd_params)
